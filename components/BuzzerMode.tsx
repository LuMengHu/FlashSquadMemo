...
export default function BuzzerMode({ questions: allQuestions }: BuzzerModeProps) {
  ...
  useEffect(() => { if (gameState === 'displaying' && currentQuestion) { if (intervalRef.current) clearInterval(intervalRef.current); intervalRef.current = setInterval(() => { setDisplayedQuestionText(prev => { if (prev.length >= currentQuestion.question.length) { if (intervalRef.current) clearInterval(intervalRef.current); setGameState('answered'); return prev; } return currentQuestion.question.substring(0, prev.length + 1); }); }, 200); } return () => { if (intervalRef.current) clearInterval(intervalRef.current) }; }, [gameState, currentQuestion]);
